...
class Session_user(MainMixin):
    ...
    ), on_delete=models.CASCADE, related_name='user_sessions', verbose_name='User')
    device = models.CharField(max_length=150, verbose_name='Device')

    def __str__(self):
        return f'id: {self.id} | user: {self.user} | device: {self.device}'
...
class Session_client(MainMixin):
    client = models.ForeignKey(Client, on_delete=models.CASCADE,
                               related_name='client_sessions', verbose_name='Client')
    device = models.CharField(max_length=150, verbose_name='Device')

    def __str__(self):
        return f'id: {self.id} | user: {self.client} | device: {self.device}'
